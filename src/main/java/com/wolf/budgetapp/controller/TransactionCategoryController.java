...
public class TransactionCategoryController {
  ...
  public ResponseEntity<List<TransactionCategoryDto>> getAllTransactionCategories() {
    return new ResponseEntity<>(
        transactionCategoryDtoMapper.toDtoList(
            transactionCategoryService.getAllTransactionCategories()),
        HttpStatus.OK);
  }

  // returns category associated with given categoryName
  @GetMapping("/{categoryName}")
  public ResponseEntity<TransactionCategoryDto> getTransactionCategoryByName(
      @PathVariable String categoryName) {
    return new ResponseEntity<>(
        transactionCategoryDtoMapper.toDto(
            transactionCategoryService.getTransactionCategoryByName(categoryName)),
        HttpStatus.OK);
  }
  ...
  public ResponseEntity<String> getDescriptionByCategoryName(@PathVariable String categoryName) {
    return new ResponseEntity<>(
        transactionCategoryService.getDescriptionByCategoryName(categoryName), HttpStatus.OK);
  }

  // adds new transaction category
  @PostMapping(consumes = MediaType.APPLICATION_JSON_VALUE)
  ResponseEntity<Object> addTransactionCategory(
      @RequestBody TransactionCategoryDto transactionCategoryDto) {
    TransactionCategory transactionCategory;
    try {
      transactionCategory =
          transactionCategoryService.createTransactionCategory(transactionCategoryDto);
    } catch (EntityNotFoundException e) {
      return new ResponseEntity<>(e.getLocalizedMessage(), HttpStatus.BAD_REQUEST);
    }
    return new ResponseEntity<>(
        transactionCategoryDtoMapper.toDto(transactionCategory), HttpStatus.OK);
  }
  ...
  ResponseEntity<Object> updateTransactionCategory(
      @PathVariable String categoryName,
      @RequestBody TransactionCategoryDto updatedTransactionCategoryDto) {
    TransactionCategory transactionCategory;
    try {
      transactionCategory = transactionCategoryService.updateTransactionCategory(
          categoryName, updatedTransactionCategoryDto);
    } catch (EntityNotFoundException e) {
      return new ResponseEntity<>(e.getLocalizedMessage(), HttpStatus.BAD_REQUEST);
    }
    return new ResponseEntity<>(
