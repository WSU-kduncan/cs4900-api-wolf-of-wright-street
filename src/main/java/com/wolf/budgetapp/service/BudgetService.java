...
public class BudgetService {
    ...
    public Budget getBudgetByEmailAndName(String email, String name) throws EntityNotFoundException {
        ...
        if (result.isEmpty()) {
            throw new EntityNotFoundException("Budget with email and name: " + email + ", " + name + " not found");
        }
        return result.get();
    }

    public Budget getExactBudget(String email, String name, LocalDate period) throws EntityNotFoundException {
        Optional<Budget> result = budgetRepository.findExactBudget(email, name, period);
        if (result.isEmpty()) {
            throw new EntityNotFoundException("Budget with email, name, period: " + email + ", " + name + ", " + period + " not found");
        }
        return result.get();
    }
    ...
    public void deleteBudget(String email, String name, LocalDate period) throws EntityNotFoundException {
        Budget budget = budgetRepository
            ...
            .orElseThrow(() -> new EntityNotFoundException("User not found with email, name, period:" + email + ", " + name + ", " + period));
